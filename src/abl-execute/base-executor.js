...
var BaseExecutor = /** @class */ (function () {
    ...
    BaseExecutor.prototype.mapResult = function (err, stdout, stderr) {
        try {
            ...
            var re_1 = /(ERROR|WARNING) File:'(.*)' Row:(\d+) Col:(\d+) Error:(.*) Message:(.*)/;
    ...
    BaseExecutor.prototype.executeCommand = function (document, procedure, params, mergeOeConfig, silent) {
        var _this = this;
        ...
        if (!wf)
            wf = extension_config_1.ExtensionConfig.getInstance().getGenericWorkspaceFolder();
        var doCommand = function () {
            if (!silent)
                _this.setDiagnostic(document, []);
            var result = _this.runProcess(procedure, params.join(','), mergeOeConfig, wf.uri.fsPath);
            return result.then(function (errors) {
                if (!silent)
                    _this.setDiagnostic(document, errors);
                if (errors.length > 0)
                    return false;
                return true;
            }).catch(function (e) {
    ...
    BaseExecutor.prototype.executeStandaloneCommand = function (procedure, params, mergeOeConfig, silent) {
        var _this = this;
        var wf;
    ...
    BaseExecutor.prototype.setDiagnostic = function (document, errors) {
        ...
        errors.forEach(function (error) {
            ...
            if (error.line > 0) {
                if (document && document.uri.path == filePath) {
                    var range_1 = new vscode.Range(error.line - 1, startColumn, error.line - 1, document.lineAt(error.line - 1).range.end.character + 1);
        ...
        diagnosticMap.forEach(function (diagMap, file) {
            if (_this.errorDiagnostic)
                _this.errorDiagnostic.set(vscode.Uri.parse(file), diagMap[vscode.DiagnosticSeverity.Error]);
            if (_this.warningDiagnostic)
                _this.warningDiagnostic.set(vscode.Uri.parse(file), diagMap[vscode.DiagnosticSeverity.Warning]);
        });
    };
    BaseExecutor.prototype.runProcess = function (procedure, param, mergeOeConfig, workspaceRoot) {
        var cmd = this.getBinary();
        var oeConfig = extension_config_1.ExtensionConfig.getInstance().getConfig(mergeOeConfig);
        var env = this.ablEnvironment.setupEnvironmentVariables(process.env, oeConfig, workspaceRoot);
        var args = this.ablEnvironment.createProArgs({
            parameterFiles: oeConfig.parameterFiles,
            configFile: oeConfig.configFile,
            batchMode: this.isBatch(),
            startupProcedure: path.join(extension_config_1.ExtensionConfig.getInstance().getExtensionPath(), "abl-src/".concat(procedure)),
            param: param,
            workspaceRoot: workspaceRoot
        });
        var cwd = oeConfig.workingDirectory ? oeConfig.workingDirectory.replace('${workspaceRoot}', workspaceRoot).replace('${workspaceFolder}', workspaceRoot) : workspaceRoot;
        var mapResult = this.mapResult.bind(this);
        return new Promise(function (resolve, reject) {
            cp.execFile(cmd, args, { env: env, cwd: cwd }, function (err, stdout, stderr) {
                try {
    ...
    BaseExecutor.prototype.saveAndExec = function (document, action) {
        if (document.isDirty) {
            return new Promise(function (resolve, reject) {
                var _a;
                (_a = vscode.window).showInformationMessage.apply(_a, __spreadArray(['Current file has unsaved changes!'], ['Save', 'Cancel'], false)).then(function (result) {
                    if (result == 'Save')
                        document.save().then(function (saved) { if (saved) {
                            action().then(function (v) { return resolve(v); });
                        } });
                    else
                        resolve(false);
                });
            ...
            return action().then(function (value) { return value; });
    };
    return BaseExecutor;
}());
