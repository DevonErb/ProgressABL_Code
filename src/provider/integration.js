...
var Integration = /** @class */ (function () {
    ...
    Integration.prototype.currentFileSaveMap = function (args) {
        ...
        var filename = null;
        if (args) {
            if (((0, util_1.isArray)(args)) && (args.length > 0))
                filename = args[0];
            else
                filename = args;
        }
    ...
    Integration.prototype.saveMapFile = function (textDocument, filename) {
        var document = source_1.AblSource.Controller.getInstance().getDocument(textDocument);
        if (document) {
            ...
            if (filename) {
                ...
                var opt = { prompt: 'Save into file', value: document.document.uri.fsPath + '.oe-map.json' };
                vscode.window.showInputBox(opt).then(function (fname) { if (fname)
                    save_1(fname, true); });
            }
        }
    };
    ...
    Integration.prototype.currentFileGetSourceCode = function () {
        var doc = vscode.window.activeTextEditor.document;
        if (doc)
            return new source_1.AblSource.Extractor().execute(doc);
        return;
    };
    ...
    Integration.prototype.compile = function (fileName, mergeOeConfig) {
        return new Promise(function (resolve) {
            vscode.workspace.openTextDocument(fileName).then(function (textDocument) {
                abl_execute_1.AblExecute.Compile.getInstance().execute(textDocument, mergeOeConfig, true, [abl_execute_1.AblExecute.COMPILE_OPTIONS.COMPILE]).then(function (v) { return resolve(v); });
            });
        });
    };
    Integration.prototype.fileGetMap = function (fsPath) {
        var document = source_1.AblSource.Controller.getInstance().getDocument(vscode.Uri.file(fsPath));
        if (document) {
            return IntegrationV1.Generate.map(document);
        }
        return {};
    ...
    var Generate = /** @class */ (function () {
        ...
        Generate.map = function (document) {
            var _tempTables = document.tempTables.map(function (tempTable) {
                var ttFields = __spreadArray(__spreadArray([], tempTable.fields, true), (tempTable.referenceFields || []), true).map(function (field) {
                    return {
                        name: field.name,
            ...
            var _includes = document.includes.filter(function (include) { return !!include.uri; }).map(function (include) {
                var _a;
                var includeMap;
                var includeDocument = source_1.AblSource.Controller.getInstance().getDocument(include.uri);
                if (includeDocument) {
                    includeMap = Generate.map(includeDocument);
                }
                return {
                    fsPath: (_a = include.uri) === null || _a === void 0 ? void 0 : _a.fsPath,
                    name: include.name,
                    map: includeMap
                };
            });
        ...
        Generate.table = function (tableName) {
            return database_1.AblDatabase.Controller.getInstance().getTable(tableName);
        };
        Generate.tables = function () {
            return database_1.AblDatabase.Controller.getInstance().getCollection().map(function (item) { return item.name; });
        };
        return Generate;
    }());
