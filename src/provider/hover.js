...
var Hover = /** @class */ (function () {
    ...
    Hover.prototype.registerCommands = function (context) {
        context.subscriptions.push(vscode.languages.registerHoverProvider(types_1.AblSchema.languageId, this));
    };
    ...
    Hover.prototype.analyseHover = function (document, position, token) {
        var statement = statement_util_1.StatementUtil.statementAtPosition(document.document, position);
        if (!statement)
            return;
        var words = statement_util_1.StatementUtil.cleanArray(statement.statement.split(/[\.\:\s\t]/));
        if (words.length > 0) {
    ...
    Hover.prototype.buildHoverFromReference = function (document, reference, statement) {
        if (types_1.AblTypeCheck.isTempTable(reference)) {
            ...
            // variable/param as buffer
                ...
                else if (reference.bufferType == types_1.AblType.BUFFER_REFERENCE.TEMP_TABLE) {
    ...
    Hover.prototype.buildHoverFromReferenceProperty = function (document, reference, name, statement) {
        var _a, _b;
        var property;
        var refData;
        if (types_1.AblTypeCheck.isTempTable(reference)) {
            refData = document.getTempTable(reference.name);
            property = document.getAllFields(refData).find(function (item) { return item.name.toLowerCase() == name.toLowerCase(); });
        }
        else if (!types_1.AblTypeCheck.isMethod(reference)) {
            // variable/param as temp-table
            if (reference.dataType == types_1.AblType.ATTRIBUTE_TYPE.TEMP_TABLE) {
            ...
            // variable/param as buffer
                ...
                if (reference.bufferType == types_1.AblType.BUFFER_REFERENCE.TABLE) {
                    ...
                    property = (_b = (_a = refData) === null || _a === void 0 ? void 0 : _a.fields) === null || _b === void 0 ? void 0 : _b.find(function (item) { return item.name.toLowerCase() == name.toLowerCase(); });
                }
                else if (reference.bufferType == types_1.AblType.BUFFER_REFERENCE.TEMP_TABLE) {
        ...
        if (property) {
            if (types_1.AblTypeCheck.isTempTable(refData)) {
                ...
                if (types_1.AblTypeCheck.isVariable(reference) && reference.dataType == types_1.AblType.ATTRIBUTE_TYPE.BUFFER) {
    ...
    Hover.prototype.buildHoverFromTable = function (table, statement, variable) {
        var _a, _b;
        var result = [];
        var pkList = (_b = (_a = table.indexes.find(function (item) { return item.isPK; })) === null || _a === void 0 ? void 0 : _a.fields) === null || _b === void 0 ? void 0 : _b.join(', ');
        if ((variable === null || variable === void 0 ? void 0 : variable.dataType) == types_1.AblType.ATTRIBUTE_TYPE.BUFFER) {
            result.push("".concat(variable.scope, " buffer **").concat(variable.name, "**\n"));
    ...
    Hover.prototype.buildHoverFromTableField = function (table, name, statement) {
        // table field
        var field = table.fields.find(function (item) { return item.name.toLowerCase() == name.toLowerCase(); });
        if (field) {
            var result = [];
            result.push("field **".concat(field.name, "**\n"));
            result.push("*".concat(field.description, "*\n"));
    ...
    Hover.prototype.buildTempTableHover = function (tempTable, statement, variable) {
        var result = [];
        if ((variable === null || variable === void 0 ? void 0 : variable.dataType) == types_1.AblType.ATTRIBUTE_TYPE.BUFFER) {
            var detail = "".concat(variable.scope, " buffer **").concat(variable.name, "**\n");
    ...
    Hover.prototype.buildMethodHover = function (method, statement) {
        ...
        result.appendMarkdown("".concat(method.visibility, " ").concat(method.type, " **").concat(method.name, "**\n"));
        if (((_a = method.params) === null || _a === void 0 ? void 0 : _a.length) > 0) {
            result.appendMarkdown('\n---\nParameters:\n');
            method.params.forEach(function (param) {
                if (param.dataType == types_1.AblType.ATTRIBUTE_TYPE.BUFFER) {
                    result.appendMarkdown("- buffer *".concat(param.name, "* for ").concat(param.bufferType, " *").concat(param.likeType, "*\n"));
                }
                else if (param.dataType == types_1.AblType.ATTRIBUTE_TYPE.TEMP_TABLE) {
                    result.appendMarkdown("- ".concat(param.direction, " for temp-table *").concat(param.name, "*\n"));
                }
                else {
                    result.appendMarkdown("- ".concat(param.direction, " *").concat(param.name, "*\n"));
                }
            });
        }
        return new vscode.Hover(result, statement.wordRange);
    ...
    Hover.prototype.buildVariableHover = function (variable, statement) {
        var result = [];
        var detail = "".concat(variable.scope, " variable **").concat(variable.name, "**\n");
        if (types_1.AblTypeCheck.isParameter(variable) && variable.direction)
            detail = "".concat(variable.direction, " ").concat(detail);
        result.push("".concat(detail, "\n"));
